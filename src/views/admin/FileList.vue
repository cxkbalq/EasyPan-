      ...
      >
        <template #fileName="{ index, row }">
          ...
          >
            ...
            >
              <icon :userId="row.userId" :cover="row.fileCover" :width="32"></icon>
            </template>
            <template v-else>
            ...
            <span class="op">
              <template v-if="row.showOp && row.fileId">
                ...
                <span class="iconfont icon-close" @click="updateUserStatus(row)" v-if="row.fengJing == 2">
                  封禁
                </span
                >
                <span class="iconfont icon-ok" @click="updateUserStatus(row)" v-if="row.fengJing == 1">
                  解封
                </span
                >
<!--                <span class="iconfont icon-del" @click="delFile(row)">-->
<!--                            <span v-if="row.status == 2" style="color: #529b2e">封禁</span>-->
<!--                    <span v-if="row.status == 0" style="color: #f56c6c">封禁中</span>-->
<!--                </span-->
<!--                >-->
              </template>
            </span>
        ...
        <template #status="{ index, row }">
          <span v-if="row.fengJing == 2" style="color: #529b2e">正常</span>
          <span v-if="row.fengJing == 1" style="color: #f56c6c">封禁中</span>
        </template>
      </Table>
    </div>
    <!--预览-->
...
const api = {
  ...
  updateFileStatus: "/admin/updateFileStatus",
...
const updateUserStatus = (row) => {
  proxy.Confirm(
      `你确定要【${row.fengJing == 2 ? "禁用" : "恢复"}】吗？`,
      async () => {
        let result = await proxy.Request({
          url: api.updateFileStatus,
          params: {
            userId: row.userId,
            status: row.fengJing == 2 ? 1 : 2,
            fileId: row.fileId
          },
        });
        if (!result) {
...
const download = async (row) => {
  ...
  window.location.href = api.download + "/" + result.data+"/"+row.userId;
};
