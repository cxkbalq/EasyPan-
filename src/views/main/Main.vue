      ...
      >
        <template #fileName="{ index, row }">
          ...
          >
            ...
            >
              <icon v-if="row.fengJing==2" :cover="row.fileCover" :width="32" :save-sf="row.saveSf"></icon>
              <icon v-if="row.fengJing==1" :fileType="row.fengJing+10" :width="32"></icon>
            </template>
            <template v-else>
              <icon v-if="row.folderType == 0 &&row.fengJing==2" :fileType="row.fileType"></icon>
              <icon v-if="row.folderType == 0 &&row.fengJing==1" :fileType="row.fengJing+10"></icon>
              <icon v-if="row.folderType == 1 &&row.fengJing==2" :fileType="0"></icon>
              <icon v-if="row.folderType == 1 && row.fengJing==1" :fileType="row.fengJing+10"></icon>
            </template>
            ...
            <span class="op">
              <template v-if="row.showOp && row.fileId && row.status == 2 &&row.fengJing==2">
...
const loadDataList = async () => {
  ...
  for (let i = 0; i < result.data.list.length; i++) {
    if(result.data.list[i].status!= 2){
     setTimeout(loadDataList, 80000); // 3秒后再次调用
      break;
    }
  }
  editing.value = false;
...
const saveNameEdit = async (index) => {
  ...
  editing.value = false;
  if(result.code==200){
    proxy.Message.success(result.data)
  }
  loadDataList();
...
const preview = (data) => {
  if(data.fengJing==1){
    proxy.Message.warning("文件违规，以封禁！");
    return;
  }
